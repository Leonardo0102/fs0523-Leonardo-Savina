const numeroCaso:number=numeroCasuale(1,100)
function numeroCasuale(min:number,max:number):number{
    return Math.floor(Math.random()*(max-min)+min);
}
console.log(numeroCaso)

function vincitore(numeroCaso:number,giocatore1:number,giocatore2:number):string|void{
...
const giocatore1:number=10
const giocatore2:number=20
const risultatoFinale:string|void=vincitore(numeroCaso, giocatore1,giocatore2)
